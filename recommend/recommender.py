...
from recommend.enums import RiskLevel
from datetime import datetime
from langchain_openai import ChatOpenAI
from langchain.schema import HumanMessage, SystemMessage

class GPTPortfolioRecommender:
    def __init__(self, dataset_path="./recommend/financial_portfolio_dataset.json", openai_api_key=None):
        print(f"GPT 포트폴리오 추천기 초기화 중... 데이터 파일: {dataset_path}")
...
"""
        ...
        try:
            print(f"GPT 자산배분 요청 중... ({risk_level.value}, {target_amount:,}원, {period}개월)")
            response = self.llm.invoke(messages)
            
            # JSON 파싱 (GPT 응답에서 JSON 부분만 추출)
            ...
            print(f"GPT 배분 결과: {final_allocation}")
            return final_allocation, reasoning
            
        except Exception as e:
            print(f"GPT 배분 추천 실패: {e}")
            # 폴백: 기본 배분
    ...
    def recommend(self, risk_level: RiskLevel, target_amount: int, period: int):
        ...
        print(f"GPT 추천 시작: {risk_level.value}, {target_amount:,}원, {period}개월")
        
        # 1. GPT로 자산배분 결정
        allocation, reasoning = self.get_gpt_allocation(risk_level, target_amount, period)
        print(f"GPT 추천 근거: {reasoning}")
            ...
            for prod in products:
                ...
                recommended[ptype].append({
                        "name": (prod.get("productName") or 
                                ...
                                prod.get("itmsNm", "상품명 없음")),  # ETF 상품명 추가
                        "bank": (prod.get("bankName") or 
                                prod.get("bondIsurNm") or 
                                prod.get("corpNm", "발행기관 없음")),  # ETF 발행사 추가
                        "rate": rate,
        ...
        result = {
            ...
            "gptReasoning": reasoning  # GPT 추천 근거 추가
        }
        
        print(f"GPT 추천 완료: 예상수익 {expected_total:,.0f}원")
        return result